...
Decimos que un conjunto _$A$ esta contenido en el conjunto $B$_ ($A$ es subconjunto de $B$) y notamos $A subset.eq B$ si se cumple $x in A => x in B$. Si $A$ no esta contenido en $B$, osea $not (A subset.eq B)$ entonces $A subset.eq.not B$.

=== Contención Estricta
Se dice que _$A$ esta contenido estrictamente en $B$_ y se nota $A subset B$ si se cumple $A subset.eq B and A != B$.
